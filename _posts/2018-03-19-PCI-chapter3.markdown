...
##  一、分级聚类
分级聚类算法（Hierarchical Cluster Analysis，HCA）通过连续不断的将最为相似的群组两两合并，来构造出一个群组的层级结构。其中的每个群组都是从一个简单元素开始的。在每次迭代的过程中，分级聚类算法会计算每两个群组间的距离，并将最近的两个群组合并成一个新的群组。这个过程一直重复下去，直到只剩下一个群组为止。
